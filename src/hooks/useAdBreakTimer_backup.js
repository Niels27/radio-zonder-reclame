...
export const useAdBreakTimer = (audioPlayer, playlistProvider = 'youtube') => {
  ...
  // ✅ NEW: Enhanced community timing state
  const [nextCommunityTiming, setNextCommunityTiming] = useState(null); // Full community timing metadata
  const [originalRadioStation, setOriginalRadioStation] = useState(null); // Track original station for nonstop mode
  
  const autoDetectorRef = useRef(null);
  const detectionTriggeredRef = useRef(false);

  const timerRef = useRef(null);
  const adBreakTimeoutRef = useRef(null);  // Get next community timing for current station - ENHANCED
        ...
        if (currentMinute < suggestions.halfHour.starts || 
          ...
          potentialTimings.push({
            ...
            duration: suggestions.halfHour.ends ? 
              Math.abs(suggestions.halfHour.ends - suggestions.halfHour.starts) * 60 : 
              adBreakDuration * 60, // fallback to user duration in seconds
            type: 'halfHour',
            startMinute: suggestions.halfHour.starts,
            endMinute: suggestions.halfHour.ends
          });
        }
      }
      ...
      if (suggestions.fullHour && suggestions.fullHour.starts) {
        ...
        potentialTimings.push({
          ...
          duration: suggestions.fullHour.ends ? 
            Math.abs(suggestions.fullHour.ends - suggestions.fullHour.starts) * 60 : 
            adBreakDuration2 * 60, // fallback to user duration in seconds
          type: 'fullHour',
          startMinute: suggestions.fullHour.starts,
          endMinute: suggestions.fullHour.ends
        });
      }

      // Return the nearest future timing with all metadata
      if (potentialTimings.length > 0) {
        potentialTimings.sort((a, b) => a.time - b.time);
  ...
  }, [useCommunityTimings, adBreakDuration, adBreakDuration2]);
  // Calculate next ad break time - ENHANCED with proper community timing support
  const getNextAdBreakTime = useCallback(async (stationName = null) => {
    ...
    if (useCommunityTimings && stationName) {
      ...
      if (communityTiming) {
        setNextCommunityTiming(communityTiming); // Store metadata
        return communityTiming.time;
      }
    }

    // Clear community timing if not found
    ...
    // If both ad breaks have passed this hour, go to next hour
    const nextBreak = new Date();
  ...
  const endAdBreak = useCallback(() => {
    ...
    }    // Resume audio with priority: queued > original > paused radio > last saved
    setTimeout(() => {
      if (queuedStation) {
      ...
      } else if (originalRadioStation) {
        // Restore original radio station (important for nonstop mode)
        console.log('🎵 Resuming original radio station:', originalRadioStation.name);
        audioPlayer.playRadio(originalRadioStation);
        setOriginalRadioStation(null); // Clear after use

        if (window.addNotification) {
          window.addNotification(`📻 Terug naar ${originalRadioStation.name}`, 'success', 2000);
        }
  ...
  }, [audioPlayer, queuedStation, setQueuedStation, originalRadioStation]);
  ...
  const startAdBreak = useCallback(async () => {
    ...
    startNonstopAdBreak,
    startLofiAdBreak,
    getAdBreakModeDescription,    handleAdBreakError,
    endAdBreak
  ]);

  // ✅ NEW: Start ad break with community-specified duration
  const startAdBreakWithCommunityDuration = useCallback(async (durationInSeconds) => {
    if (isAdBreakActive || audioPlayer.isTransitioning) return;

    const isRadioPlaying = audioPlayer.isPlaying && audioPlayer.currentStation && audioPlayer.currentSource === 'radio';
    const durationMinutes = Math.round(durationInSeconds / 60);

    console.log(`🌐 Starting community-timed ad break: ${durationMinutes} minutes (${durationInSeconds}s)`);
    setIsAdBreakActive(true);
    setShouldPlayPlaylistDuringAdBreak(isRadioPlaying);
    window.isAdBreakActive = true;
    window.currentAdBreakTimeLeft = durationInSeconds;

    // Set timeout for community duration
    adBreakTimeoutRef.current = setTimeout(() => {
      endAdBreak();
    }, durationInSeconds * 1000);

    if (isRadioPlaying) {
      console.log('🎵 Pausing radio for community-timed ad break');
      audioPlayer.pauseRadioForAdBreak();

      setTimeout(async () => {
        try {
          if (adBreakMode === 'playlist') {
            await startPlaylistAdBreak(durationMinutes);
          } else if (adBreakMode === 'nonstop') {
          ...
          console.error('Failed to start community ad break:', error);
          handleAdBreakError();
        }
      }, 500);
    }
  }, [isAdBreakActive, audioPlayer, adBreakMode, startPlaylistAdBreak, startNonstopAdBreak, startLofiAdBreak, handleAdBreakError, endAdBreak]);
    ...
    try {      if (isManualTestActive) {
        ...
        if (isRadioPlaying) {
          if (isLongDuration) {
            ...
            console.log('🎵 Pausing radio voor manual test');
            audioPlayer.pauseRadioForAdBreak();
          }
  ...
  }, [audioPlayer, adBreakMode, playlistUrl, playlistShuffle, playlistProvider, isManualTestActive, setIsManualTestActive, setShouldPlayPlaylistDuringAdBreak, startNonstopAdBreak, startLofiAdBreak, getAdBreakModeDescription, extractPlaylistId]);  // Check ad break time - ENHANCED with proper community timing and duration handling
  const checkAdBreakTime = useCallback(async () => {
        ...
        if (suggestions) {
          ...
          if (suggestions.halfHour && suggestions.halfHour.starts) {
            ...
            if (currentMinute === timingMinute && currentSecond >= 0 && currentSecond <= 5) {
              ...
              // Calculate community timing duration
              if (suggestions.halfHour.ends) {
                communityDuration = Math.abs(suggestions.halfHour.ends - suggestions.halfHour.starts) * 60; // Convert to seconds
              }
              
              console.log(`🌐 Community timing trigger (half-hour) at ${currentMinute}:${String(currentSecond).padStart(2, '0')} for ${audioPlayer.currentStation.name}, duration: ${communityDuration || 'fallback'}`);
            }
          }
          ...
          if (!shouldStartAdBreak && suggestions.fullHour && suggestions.fullHour.starts) {
            ...
            if (currentMinute === timingMinute && currentSecond >= 0 && currentSecond <= 5) {
              ...
              // Calculate community timing duration
              if (suggestions.fullHour.ends) {
                communityDuration = Math.abs(suggestions.fullHour.ends - suggestions.fullHour.starts) * 60; // Convert to seconds
              }
              
              console.log(`🌐 Community timing trigger (full-hour) at ${currentMinute}:${String(currentSecond).padStart(2, '0')} for ${audioPlayer.currentStation.name}, duration: ${communityDuration || 'fallback'}`);
            }
          }
        }
      } catch (error) {
    ...
    if (shouldStartAdBreak) {
      ...
      // Store original radio station for proper restoration (especially important for nonstop mode)
      if (audioPlayer.currentStation && audioPlayer.currentSource === 'radio') {
        setOriginalRadioStation(audioPlayer.currentStation);
        console.log(`📻 Storing original radio station: ${audioPlayer.currentStation.name}`);
      }
      ...
      if (window.addNotification) {
        if (usedCommunityTiming) {
          ...
          window.addNotification(
            `🎯 Eigen timing gebruikt voor switching`, 
            'info', 
            2000
          );
        }
      ...
      // Start ad break with community duration if available
      if (usedCommunityTiming && communityDuration) {
        console.log(`🌐 Using community duration: ${communityDuration} seconds`);
        await startAdBreakWithCommunityDuration(communityDuration);
      } else {
        await startAdBreak();
  ...
  }, [isTimerRunning, isAdBreakActive, adBreakMinute, adBreakMinute2, useCommunityTimings, audioPlayer, startAdBreak, startAdBreakWithCommunityDuration]);

  // Timer effect - updated to support async community timings
  useEffect(() => {
