...
class MusicDetector {
  ...
  // ✅ ENHANCED: Detection result with rhythm info
  createDetectionResult(isMusic, confidence, detectionMethod, musicProbability) {
    return {
      ...
      // ✅ ENHANCED: More detailed debugging info
      silenceInfo: {
      ...
      // Trigger ad break if 3+ silences detected
      triggerAdBreak: this.consecutiveSilences >= 3
    };
  }

  // ✅ EXISTING: Main classification method (now enhanced)
  async classifyAudio() {
    if (!this.analyser) {
      this.status = 'error';
      return null;
...
stopDetection() {
  ...
  // ✅ CRITICAL FIX: Store MULTIPLE volume references before any cleanup
  const audioVolume = this.audioElement?.volume || 0.7;
  const wasPlaying = this.audioElement?.paused === false;
  
  // ✅ NEW: Get UI volume from global state if available
  const uiVolume = window.audioPlayer?.volume || audioVolume;
  const useVolume = Math.max(audioVolume, uiVolume); // Use the higher value to be safe
  
  console.log(`🔧 Preserving volume during cleanup: Audio=${Math.round(audioVolume * 100)}%, UI=${Math.round(uiVolume * 100)}%, Using=${Math.round(useVolume * 100)}%`);
  
  // Clean up audio analysis connections but preserve audio output and volume
  if (this.analyser) {
    try {
      this.analyser.disconnect();
  ...
  // ✅ CRITICAL FIX: BULLETPROOF volume restoration with multiple safety nets
  if (this.audioElement) {
    // SAFETY NET 1: Immediate restoration
    setTimeout(() => {
      if (this.audioElement) {
        console.log(`🔊 SAFETY NET 1: Setting volume to ${Math.round(useVolume * 100)}%`);
        this.audioElement.volume = useVolume;
        
        // Update UI volume if possible
        if (window.audioPlayer?.setVolume && Math.abs(window.audioPlayer.volume - useVolume) > 0.01) {
          console.log(`🔊 SAFETY NET 1: Updating UI volume to ${Math.round(useVolume * 100)}%`);
          window.audioPlayer.setVolume(useVolume);
        }
      }
    }, 50);
    
    // SAFETY NET 2: Delayed verification and correction
    setTimeout(() => {
      if (this.audioElement) {
        const currentVolume = this.audioElement.volume;
        const currentUIVolume = window.audioPlayer?.volume || currentVolume;
        
        if (Math.abs(currentVolume - useVolume) > 0.01 || Math.abs(currentUIVolume - useVolume) > 0.01) {
          console.warn(`🚨 SAFETY NET 2: Volume drift detected! Audio=${Math.round(currentVolume * 100)}%, UI=${Math.round(currentUIVolume * 100)}%`);
          console.log(`🔧 SAFETY NET 2: Force correcting to ${Math.round(useVolume * 100)}%`);
          ...
          if (window.addNotification) {
            window.addNotification(`🔊 Volume automatisch hersteld naar ${Math.round(useVolume * 100)}%`, 'info', 1500);
          }
        }
        
        // ✅ CRITICAL FIX: Don't force play - let the audio player handle playback state
        if (wasPlaying && this.audioElement.paused) {
          console.log('🔊 Audio was playing before detection test - keeping playback state');
          // Let the parent audio player handle this, don't force play here
        }
      }
    }, 200);
    
    // SAFETY NET 3: Final verification
    setTimeout(() => {
      if (this.audioElement && window.audioPlayer) {
        const finalAudioVolume = this.audioElement.volume;
        const finalUIVolume = window.audioPlayer.volume;
        
        console.log(`🔊 SAFETY NET 3: Final volume check: Audio=${Math.round(finalAudioVolume * 100)}%, UI=${Math.round(finalUIVolume * 100)}%`);
        
        if (Math.abs(finalAudioVolume - finalUIVolume) > 0.01) {
          console.warn(`🚨 SAFETY NET 3: Volume still desynced! Final correction...`);
          this.audioElement.volume = finalUIVolume;
        }
      }
    }, 500);
  }
  ...
  console.log('🔧 Enhanced detection stopped, volume preservation applied with 3 safety nets');
}
  // ✅ FIXED: Start detection with better audio element tracking
  startDetection(audioElement, callback, isTestMode = false) {
    this.detectionCallback = callback;
    this.audioElement = audioElement; // ✅ Store reference for cleanup
    this.consecutiveNoMusicCount = 0;
    this.consecutiveSilences = 0;
    this.detectionHistory = [];
    this.silenceHistory = [];
    ...
    console.log('🎯 Enhanced silence + rhythm based ad detection started');
    console.log(`🔇 Silence thresholds: ${this.silenceMinDuration}s - ${this.silenceMaxDuration}s`);
    console.log('📢 Will trigger ad break after 3 consecutive ad-like silences');
    console.log('🥁 Rhythm detection: Analyzing beat patterns and music characteristics');
...
export const setupAdDetection = async (audioElement, adBreakCallback) => {
  ...
  console.log('🔧 Initializing SMART ad detector with ±5 minute detection windows...');
  await detector.initialize();
  
  detector.startDetection(audioElement, (result) => {
    console.log('🎯 Detection result in 5-min window system:', {
      type: result.isMusic ? '🎵 Music' : '📢 NOT Music (Ads)',
      confidence: `${Math.round(result.confidence * 100)}%`,
      silences: `${result.silenceInfo.consecutiveSilences}/3`,
      audioLevel: result.silenceInfo.lastAudioLevel.toFixed(2),
      rhythm: result.silenceInfo.rhythmScore.toFixed(1),
      bpm: result.silenceInfo.averageBPM,
      musicProb: `${Math.round(result.silenceInfo.musicProbability * 100)}%`,
      windowLogic: result.isMusic ? 'Can end ad break' : 'Can start ad break'
    });
    
    // Pass result to the smart handler with 5-minute window logic
...
export const setupTestDetection = async (audioElement, statusCallback) => {
  if (globalDetector) {
    ...
    await new Promise(resolve => setTimeout(resolve, 500)); // ✅ Longer delay for proper cleanup
  }
  
  globalDetector = new MusicDetector();
  
  console.log('🧪 Initializing enhanced test detector with rhythm analysis...');
