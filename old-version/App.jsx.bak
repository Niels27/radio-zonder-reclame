...
function App() {
  ...
  // Fade audio streams setting (default OFF)
  const [fadeAudioStreams, setFadeAudioStreams] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('fade_audio_streams') || 'false');
    } catch {
      return false;
    }
  });
  
  const audioPlayer = useAudioPlayer(playlistProvider, autoCloseOverlays, fadeAudioStreams);
  ...
  // Save fade audio streams setting to localStorage
  useEffect(() => {
    localStorage.setItem('fade_audio_streams', JSON.stringify(fadeAudioStreams));
  }, [fadeAudioStreams]);
        ...
        <div className="flex-1 flex flex-col pb-32"> {/* Add bottom padding for fixed footer */}          {/* Playlist Provider Selection Section - Only show for playlist mode */}
          ...
          <div className="bg-gray-800 border-b border-gray-700">       <ErrorBoundary>
            <AdBreakSettings
              ...
              onVisualizerBlurChange={setVisualizerBlur}              // ✅ NEW: Auto-close overlays setting
              ...
              onFadeAudioStreamsChange={setFadeAudioStreams}
